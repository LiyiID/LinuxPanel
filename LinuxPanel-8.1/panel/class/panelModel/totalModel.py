...
class main(panelBase):
    ...
    def _create_table(self, get):
        ...
        ts.table("sqlite_master").db(table_name).field('name').where("type=? and name=?", ('table', table_name))
        res = ts.count()
        ts.close()
        if not res:
            return False
    ...
    def _remove_all_table(self, get):
        ...
        tables_list = ts.table("sqlite_master").db('flow_day_limit').field('name').where("type=?", ('table',)).select()
