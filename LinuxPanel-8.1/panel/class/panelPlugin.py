...
class panelPlugin:
        ...
        if not cache.get(pkey):
            ...
            try:
                ...
                if 'Name or service not known' in str_ex:
                    ...
                    public.returnMsg(False, '连接下载节点失败，请尝试到【面板设置】页面切换通讯节点，或检查服务器网络是否正常: <p>Error: {}</p>'.format(str_ex))
            finally:
    ...
    def _get_flush_time(self):
        try:
            if os.path.exists(self.__plugin_save_file):
                return os.path.getmtime(self.__plugin_save_file)
        except:pass
        return 0
                ...
                try:
                    ...
                    for m_info in softList['m_list']:
                        ...
                        if n_info:
                            ...
                            ps = re.sub('<[0-9a-zA-Z\.\"\/\:\=\s\-_\?;@&\'\%\+\#]+>', '', m_info['ps'].lower())
    ...
    def sord_by_sordid(self, sList, query_str=None):
        ...
        for i in range(len(sList)):
            ...
            elif sList[i]['name'].find(query_str) != -1 and re.search(
                        r'^' + query_str.upper(), sList[i]['title']
                    ) and sList[i]['type'] == 5:
                sList[i]['sortid'] = 10
            elif sList[i]['title'].find(query_str.upper()) != -1 and re.search(
                        r'^' + query_str.upper(), sList[i]['title']
                    ) and sList[i]['type'] == 5:
                sList[i]['sortid'] = 10
    ...
    def set_coexist(self, sList):
        ...
        no_priority_php = []
        php_rep = r"^php-\d{0,2}\.\d{0,2}(\.\d{0,2})?$"
        for sInfo in sList:
            ...
            for versionA in sInfo['versions']:
                try:
                    ...
                    is_php = bool(re.match(php_rep, sTmp['name']))
            ...
            if softInfo['name'] == sName:
                if sName == 'phpmyadmin':
                    ...
                    if softInfo['ext'] and pmd:
                        softInfo['ext']['url'] = 'http://' + public.GetHost(
                        ) + ':' + pmd[1] + '/' + pmd[0]
                if "php-" in sName:
    ...
    def get_icon(self, name, downFile=None):
        ...
        for ico_type in ['svg', 'png']:

            iconFile = 'BTPanel/static/img/soft_ico/ico-{}.{}'.format(name, ico_type)
            if not os.path.exists(iconFile):
                public.run_thread(self.download_icon, (name, iconFile, downFile, ico_type))
            else:
                size = os.path.getsize(iconFile)
                if size == 0 and not iconFile.endswith(".svg"):
                    public.run_thread(self.download_icon, (name, iconFile, downFile, ico_type))
                # self.download_icon(name,iconFile,downFile)

    # 下载图标
    def download_icon(self, name, iconFile, downFile, ico_type='png'):
        ...
        if os.path.exists(srcIcon):
            ...
            public.ExecShell(r"\cp  -a -r " + srcIcon + " " + vue_iconFile)
        else:
            if downFile:
                public.ExecShell('wget -O ' + iconFile + ' ' + public.GetConfigValue('home') + downFile + " &")
            else:
                public.ExecShell('wget -O ' + iconFile + ' ' + public.get_url() + '/install/plugin/' + name + '/icon.' + ico_type + " &")
    ...
    def creatab_open_total_table(self, sql):
        '''
            ...
            @return void
        '''
        if not public.M('sqlite_master').db('open_total').where(
...
{% endblock %}'''
    ...
    def push_plugin(self, get):
        config_popular_searches = os.path.join(public.get_panel_path(), "config/popular_searches.json")
        data_popular_searches = os.path.join(public.get_panel_path(), "data/popular_searches.json")
        ...
        try:
            ...
            if not isinstance(data["push_plugin"], list):
    ...
    def check_install_lib(self, mtype):
        ...
        need = False
        # Install_Lib
        if not os.path.exists("/etc/bt_lib.lock"):
            need = True
        # # Install_Openssl
