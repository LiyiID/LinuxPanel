...
class send_to_user:
    ...
    def __init__(self):
        ...
        if not public.M('sqlite_master').db('send_settings').where('type=? AND name=?', ('table', 'send_settings')).count():
            public.M('').db('send_settings').execute('''CREATE TABLE "send_settings" (
        ...
        if not public.M('sqlite_master').db('send_msg').where('type=? AND name=?', ('table', 'send_msg')).count():
            public.M('').db('send_msg').execute('''CREATE TABLE "send_msg" ("id" INTEGER PRIMARY KEY AUTOINCREMENT,"name" TEXT,"send_type" TEXT,"msg" TEXT,"is_send" TEXT,"type" TEXT,"inser_time" TEXT DEFAULT '');''')
