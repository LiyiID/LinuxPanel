...
class panel_search:
    ...
    def __init__(self):
        ...
        if not public.M('sqlite_master').db('panel_search_log').where('type=? AND name=?', ('table', 'panel_search_log')).count():
            ...
            public.M('sqlite_master').db('panel_search_log').execute(csql,())
