...
def check_openssl_version():
    tips_file = '/tmp/openssl_version_tips.pl'
    if os.path.exists(tips_file): 
        # 10分钟内检查过openssl版本则不再检查
        if os.path.getmtime(tips_file) < time.time() - 600:
            return
    python_bin = public.get_python_bin()
    out, error = public.ExecShell('{} -c "import OpenSSL"'.format(python_bin))
    if error.find("Traceback") != -1:
        public.ExecShell("nohup {} -m pip install -I pyopenssl &>/dev/null &".format(python_bin))
        public.writeFile(tips_file, '1')
...
def panel_create_chunk():
    ...
    if free_size:
        use_size = float(free_size) * 0.05  # 取可用空间的5%
