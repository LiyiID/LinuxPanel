...
class main(projectBase):
    ...
    def get_project_stat(self, project_info, has_load_info=False):
        '''
            @name 获取项目状态信息
            @author hwliang<2021-08-09>
            @param project_info<dict> 项目信息
            @param has_load_info<bool> 是否获取项目信息
            @return list
        '''
        project_info['project_config'] = json.loads(project_info['project_config'])
...
echo $! > {pid_file}'''.format(
    ...
    def project_add_domain(self, get):
        ...
        flag = False
        res_domains = []
        if not hasattr(get,"project_name"): return public.returnMsg(False,"缺少参数! project_name")
    ...
    def modify_project(self, get):
        ...
        self.set_config(get.project_name)
        try:
            if int(project_find['project_config']['web_log']) == 0:
                ps = panelSite.panelSite()
    ...
    def list_go_sdk(self, get: public.dict_obj) -> Dict:
        ...
        return {"status": True, "installed": installed, "sdk": sdk, "used": pygvm.now_version}
    ...
    def set_go_environment(self, get):
        ...
        import subprocess
        # 解析前端发送的数据
        go_version_name = get.name
        go_path = f"/usr/local/go/{go_version_name}/bin/go"
        ...
        # 获取GOROOT的新值
        new_go_root = os.path.dirname(os.path.dirname(go_path))

        # 写入用户的shell配置文件
        bashrc_path = os.path.expanduser('~/.bashrc')
        go_root_line = f"export GOROOT={new_go_root}"
        path_line = f"export PATH=$GOROOT/bin:$PATH"

        # 读取.bashrc文件并更新GOROOT和PATH
        with open(bashrc_path, 'r') as file:
            lines = file.readlines()

        # 检查是否已经设置了GOROOT和PATH
        with open(bashrc_path, 'w') as file:
            go_root_set = False
            path_set = False
            for line in lines:
                if 'export GOROOT=' in line:
                    file.write(go_root_line + '\n')
                    go_root_set = True
                elif 'export PATH=' in line and '$GOROOT/bin' in line:
                    if go_root_set:
        ...
        # 删除旧的符号链接
        subprocess.run(['sudo', 'rm', '/usr/local/btgo'], check=True)

        # 创建新的符号链接到正确的Go版本
        subprocess.run(['sudo', 'ln', '-s', new_go_root, '/usr/local/btgo'], check=True)

        return f"环境变量GOROOT已永久更新为：{new_go_root}"
