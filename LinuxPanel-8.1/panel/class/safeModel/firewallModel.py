...
class main(safeBase):
    ...
    def create_rules(self, get):
        ...
        if types == 'accept':
            ...
            strategy = "禁止"
        public.WriteLog("系统防火墙", "添加端口规则: 协议:{}, 端口:{}, 策略:{}, IP:{}".format(protocol, ports, strategy, log_ip))
        # 如果有忽略的端口，返回忽略的端口
        if ignore_list:
    ...
    def remove_rules(self, get):
        ...
        if types == 'accept':
            ...
            strategy = "禁止"
        public.WriteLog("系统防火墙", "删除端口规则: 协议:{}, 端口:{}, 策略:{}, IP:{}".format(get.protocol, get.ports, strategy, log_ip))
    ...
    def modify_rules(self, get, addtime=None):
        ...
        if get.types == 'accept':
            ...
            strategy = "禁止"
        public.WriteLog("系统防火墙", "修改端口规则： 协议:{}, 端口:{}, 策略:{}, IP:{}".format(get.protocol, get.ports.strip(), strategy, log_ip))
    ...
    def modify_ip_rules(self, get):
        ...
        if get.types == 'accept':
            ...
            strategy = "禁止"
        public.WriteLog("系统防火墙", "修改规则, IP:{}, 策略:{} -> IP:{}, 策略:{}".format(_address, old_strategy, get.address.strip(), strategy))
        return public.returnMsg(True, '操作成功')
    ...
    def create_forward(self, get):
        ...
        self.FirewallReload()
        public.WriteLog("系统防火墙", "添加端口转发规则: 起始端口:{}, 目的端口:{}, 目的IP:{}".format(s_port, d_port, d_ip))
        return public.returnMsg(True, 'ADD_SUCCESS')
    ...
    def remove_forward(self, get):
        ...
        self.FirewallReload()
        public.WriteLog("系统防火墙", "删除端口转发规则: 起始端口:{}, 目的端口:{}, 目的IP:{}".format(s_port, d_port, d_ip))
        return public.returnMsg(True, 'DEL_SUCCESS')
    ...
    def modify_forward(self, get):
        ...
        self.FirewallReload()
        public.WriteLog("系统防火墙", "修改端口转发规则: 起始端口:{}, 目的端口:{}, 目的IP:{} -> 起始端口:{}, 目的端口:{}, 目的IP:{}".format(start_port, ended_port, ended_ip, s_port, d_port, d_ip))
        return public.returnMsg(True, '操作成功.')
    ...
    def verify_ip(self, ip_entry):
        ...
        try:
            ...
            release_ips = [IPy.IP("127.0.0.1"), IPy.IP("172.16.1.1"), IPy.IP("10.0.0.1"), IPy.IP("192.168.0.0"),
    ...
    def create_countrys(self, get):
        try:
            if not hasattr(get, 'country'):
                return public.returnMsg(False, '请输入国家名称！')
    ...
    def check_table(self):
        if public.M('sqlite_master').db('firewall_new').where('type=? AND name=?', ('table', 'firewall_new')).count():
            if public.M('sqlite_master').db('firewall_ip').where('type=? AND name=?', ('table', 'firewall_ip')).count():
                if public.M('sqlite_master').db('firewall_trans').where('type=? AND name=?', ('table', 'firewall_trans')).count():
                    if public.M('sqlite_master').db('firewall_country').where('type=? AND name=?', ('table', 'firewall_country')).count():
                        return True
        return Sqlite()
    ...
    def get_port_info(self, get):
        ...
        result_list.append({"name": "FTP主动端口", "status": 0, "port": ftp_port})
...
class Sqlite():
    ...
    def create_table(self):
        ...
        public.M(None).db('firewall_country').execute('''CREATE TABLE IF NOT EXISTS "firewall_country" (
            ...
            "brief" TEXT DEFAULT '',
            "addtime" TEXT DEFAULT '');''')
        public.M(None).db('firewall_country').execute('CREATE INDEX firewall_country_name ON firewall_country (country);')
