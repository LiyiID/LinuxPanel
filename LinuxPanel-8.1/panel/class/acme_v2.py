...
class acme_v2:
    ...
    def write_ngin_authx_file(self, auth_to, token, acme_keyauthorization, index):
        ...
        rec = re.compile(r"#CERT-APPLY-CHECK--START(?P<target>(.|\n)*?)#CERT-APPLY-CHECK--END[^\n]*\n(?P<other>(\n|.)*?)#SSL-START +SSL")
        check = rec.search(nginx_conf)

        if check is None:
            ssl_line = re.search(r"\n[ \t]*#SSL-START +SSL", nginx_conf)
            if ssl_line is None:
                raise Exception("配置文件中找不到ssl配置项，无法定位验证文件配置项的位置。")
            ...
            new_conf = re.sub(r"\n[ \t]*#SSL-START +SSL", default_cert_apply_check + ssl_line.group(), nginx_conf,1)
            public.writeFile(nginx_conf_path, new_conf)
            isError = public.checkWebConfig()
            if isError is not True:
                public.writeFile(nginx_conf_path, nginx_conf)
                raise Exception("写入验证文件失败,无法添加配置信息")

        elif check.group("other").strip() or not bool(check.group("target").strip()):
            raise Exception("配置文件中用于文件验证的部分须位于ssl配置项之前，以保证优先级，请勿调整")
...
'''
    ...
    def check_auth_env(self,args):
        ...
        if args.auth_type in ['http', 'tls'] and public.get_webserver() == "nginx":  # nginx 可以不管其他情况直接验证
            is_error = public.checkWebConfig()
            if is_error is not True:
                error_msg = (
                        '服务器配置文件存在问题，无法重载nginx配置文件，请先解决该问题，再使用文件验证申请证书，错误信息'
                        '如下：<br>ERROR:<br><font style="color:red;">' + is_error.replace("\n", '<br>') + '</font>'
                )
    ...
    def renew_cert(self, index):
        ...
        try:
            ...
            for index in order_index:
                ...
                if len(domains) == 0:
                    write_log("|-第 {} 张证书下的域名全部未使用(这些域名是:[%s])，已跳过。".format(n, ",".join(self._config['orders'][index]['domains'])))
                    continue
                else:
