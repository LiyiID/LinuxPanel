  ...
  `addtime`			INTEGER
    ...
    def _zip(self, path, sfile, dfile, log_file, z_type='tar.gz'):
        ...
        if z_type == 'zip':
            ...
            public.ExecShell("cd '{}' && zip '{}' -r {} &> {}".format(path,dfile,sfiles,log_file))
        elif z_type == 'tar.gz':
            # public.ExecShell("cd '" + path + "' && tar -zcvf '" + dfile + "' " + sfiles + " &> " + log_file)
            public.ExecShell("cd '{}' && tar -zcvf '{}' {} &> {}".format(path,dfile,sfiles,log_file))
        elif z_type == 'rar':
            rar_file = '/www/server/rar/rar'
            if not os.path.exists(rar_file):
                self.install_rar()
            # public.ExecShell("cd '" + path + "' && "+rar_file + " a -r '" + dfile + "' " + sfiles + " &> " + log_file)
            public.ExecShell("cd '{}' && {} a -r '{}' {} &> {}".format(path,rar_file,dfile,sfiles,log_file))
        elif z_type == '7z':
    ...
    def _unzip(self, sfile, dfile, password, log_file):
        ...
        if sfile.endswith('.zip'):
            ...
            public.ExecShell("unzip -P '{}' -o '{}' -d '{}' &> {}".format(password,sfile,dfile,log_file))
        #elif sfile[-7:] == '.tar.gz' or sfile[-4:] == '.tgz' or (tarfile.is_tarfile(sfile) and sfile[-3:] == '.gz'):
        elif sfile.endswith('.tar.gz') or sfile.endswith('.tgz') or (tarfile.is_tarfile(sfile) and sfile.endswith('.gz')):
            # public.ExecShell("tar zxvf '" + sfile + "' -C '" + dfile + "' &> " + log_file)
            public.ExecShell("tar zxvf '{}' -C '{}' &> {}".format(sfile,dfile,log_file))
        # elif sfile[-4:] == '.rar':
        elif sfile.endswith('.rar'):
            ...
            public.ExecShell('{} x {} -u -y "{}" "{}" &> {}'.format(rar_file,pass_opt,sfile,dfile,log_file))

        # elif sfile[-4:] == '.war':
        elif sfile.endswith('.war'):
            # public.ExecShell("unzip -P '"+password+"' -o '" + sfile + "' -d '" + dfile + "' &> " + log_file)
            public.ExecShell("unzip -P '{}' -o '{}' -d '{}' &> {}".format(password,sfile,dfile,log_file))
        # elif sfile[-4:] == '.bz2':
        elif sfile.endswith('.bz2'):
            # public.ExecShell("tar jxvf '" + sfile + "' -C '" + dfile + "' &> " + log_file)
            public.ExecShell("tar jxvf '{}' -C '{}' &> {}".format(sfile,dfile,log_file))
        # elif sfile[-3:] == '.7z':
        elif sfile.endswith('.7z'):
            ...
            public.ExecShell("gunzip -c '{}' > '{}'".format(sfile,sfile[:-3]))
        ...
        if log_msg:
            if log_msg.find("incorrect password") != -1 \
                ...
                public.ExecShell("tar xvf '{}' -C '{}' &> {}".format(sfile,dfile,log_file))
