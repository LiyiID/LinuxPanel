...
class backup:
    ...
    def backup_path(self, spath, dfile=None, exclude=[], save=3, echo_id=None):
        try:
            ...
            if not dfile:
                ...
                dfile = os.path.join(self._BACKUP_DIR, 'path', fname)
    ...
    def backup_site(self, siteName, save=3, exclude=[], echo_id=None):
        ...
        if echo_id is None:
            echo_id = self.echo_id
        self.cron_info = public.M('crontab').where('echo=?', (echo_id,)).field(
            'backupto,name,save_local,notice,notice_channel,id,sType,split_type,split_value,db_backup_path').select()[0]
        ...
        if self._cloud:
            ...
            if backup_size >= self._SPLIT_SIZE and self.cron_info.get("split_type") and self.cron_info.get("split_value") and (
                ...
                if self._cloud._title in ["AWS S3对象存储", "天翼云ZOS"]:  ## 由于aws s3上传函数独特，单独处理
...
请尽快处理""".format(
    ...
    def split_upload_file(self, backup_path: str, upload_path: str) -> Tuple[bool, str]:
        ...
        if self.cron_info.get("split_type") == "size":
            is_status, data = files.split_file(
