...
class main(logsBase):
    ...
    def get_panel_error_logs(self,get):
        ...
        if not os.path.exists(filename):
            return public.returnMsg(False,'没有找到错误日志')
    ...
    def get_slow_logs(self,get):
        ...
        if 'search' not in get:
            return public.returnMsg(True, public.xsssec(public.GetNumLines(path, limit)))
        else:
            result = []
            for line in public.GetNumLines(path, limit).split("\n"):
                if search.lower() in line.lower():
                    result.append(public.xsssec(line))

            # result = re.findall(search,public.xsssec(public.GetNumLines(path, limit)))
