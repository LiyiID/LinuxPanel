...
class system:
    ...
    def ServiceAdmin(self, get=None):
        ...
        # nginx端口被占用的情况
        if get.name == 'nginx' and  "failed" in result[1]:
            if "Address already in use" in result[1]:
                try:
                    port = result[1].split('0.0.0.0:')[1].split(" ")[0]
                    cmd = "lsof -F c -i:{}".format(port)
                    info_list = public.ExecShell(cmd)[0].strip("\n").split("\n")
                    return public.returnMsg(False, "Nginx启动错误: <br><a style='color:red;'>端口【{}】被【{}】进程占用".format(port, info_list[1][1:]))
                except Exception as e:
                    return public.returnMsg(False, "Nginx启动错误: 端口被占用，详情：{}".format(result[1]))
    ...
    # 安装任务检测
    def InstallCehck(self, get=None):
        tip_file = '/dev/shm/.panelTask.pl'
        try:
            if os.path.exists(tip_file):
                is_install=public.M('tasks').where("status=?", ('-1')).getField("id")
                if is_install:
                    return True
        except:
            pass
    ...
    # 重启面板
    def ReWeb(self, get):
        is_install=self.InstallCehck()
        if is_install == True:
            return public.returnMsg(False, '当前有安装任务正在执行，请在安装结束后再重启面板')
    ...
    def RepPanel(self, get):
        is_install=self.InstallCehck()
        if is_install == True:
            return public.returnMsg(False, '当前有安装任务正在执行，请在安装结束后再修复面板')
