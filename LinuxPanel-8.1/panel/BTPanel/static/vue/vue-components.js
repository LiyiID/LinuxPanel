dynamic.delay('vue-components', () => {
                ...
                Vue.component('bt-text', {
                    template: `<div :class="{inlineBlock:!block,block:block,relative:iconInternal}" v-if="type === 'text' || type === 'number'">
                        ...
                        <span :class="'glyphicon glyphicon-'+ icon +' cursor'" v-if="icon" @click="$emit('icon-event')" :style="iconInternal?'':'color:black'"></span>
                ...
                `,
                    ...
                    methods: {
                        ...
                        input_event: function (ev) {
                            ...
                            this.$emit('change', ev.target.checked);
                            this.value = ev.target.checked ? 1 : 0;
                        },
                    },
                });
                ...
                Vue.component('bt-button', {
                    ...
                    props: {
                        ...
                        disabled: {
							...
							Vue.component('bt-button-select', {
								template: `<div class="btn-group" :class="show?' open':''">
                   <button type="button" :class="'btn btn-'+ size+ ' btn-default base-btn'" style="height: 100%;" @click="javaEvnShowFun">{{ title }}</button>
                   <button type="button" :class="'btn btn-'+ size+ ' btn-default dropdown-toggle '" @mouseenter="show=true" @mouseleave="show=false" style="display: flex;align-items: center;justify-content: center;height: 100%;">
                   ...
                   <ul class="dropdown-menu bt_select_list " @mouseenter="show=true" @mouseleave="show=false">
                       <li>
                          <a @click="javaEnvJDK($event)">JDK管理</a>
                       </li>
                       <li>
                           <a @click="tomcat_manage_view($event)">Tomcat管理</a>
                       </li>
                   </ul>
                  </div>`,
									props: {
									...
									methods:{
										javaEnvJDK(e){
											this.show = false
											this.javaEvnShowFun()
											site.javaModle.jdk_manage_view()
											site.javaModle.javaEnvName = 'JDKConfig'
										},
										tomcat_manage_view(e){
												this.show = false
												this.javaEvnShowFun()
												site.javaModle.tomcat_manage_view()
												site.javaModle.javaEnvName = 'tomcatconfig'
										},
										javaEvnShowFun(){
												site.javaModle.javaEnvShow= true
										}
									}
							})
               Vue.component('bt-help', {
                ...
                Vue.component('bt-link', {
									template: `<div v-if="!ispath" class="inlineBlock">\
									<a :href="href" :class="{'bt_success':status,'bt_danger':!status,'bt_warning':status === 2,'status_tips':name}" :data-status="status" :data-name="name" data-type="java" :data-project="id" @click="$emit('click')" :target="target" :title="tips || title">
												<span>{{ title }}</span>
												<span :class="'glyphicon glyphicon-' + icon " v-if="icon"></span>
												</a>
											</div>
											<span v-else class="dir_details_span">
											<a v-if="isInfoData" class="btlink path" @click="openPath(title)" href="javascript:;" :title="title">{{title}}</a>
											<div v-else class="dir_details" :data-path="title">
												<a class="btlink details_event" href="javascript:;" title="点击查看目录详情">详情</a>
            ...
            <!--            </div>-->
                ...
                Vue.component('bt-layer', {
										template: `<div :id="random" :class="((className?(className + ' bt-layer-view'):'')  || {pd20:true,pb70:!btn,'bt-layer-view':true})" v-if="this.status" v-cloak>
												<slot></slot>
										</div>`,
                    props: {
                        title: {
                    ...
                    methods: {
                        ...
                        create() {
                            ...
                            this.$nextTick(() => {
								...
								Vue.component('bt-layer-btn-confirm', {
									template:`
									<div class="bt-layer-btn-confirm">
										<bt-button id="btn-confirm" type="success" :title="title" class-name=" mr0" @click="open"></bt-button>
										<div id="confirm-content" v-show="false">
											<div class="pd15" ref="content" style="color: #0f0f0f">
												<slot v-bind:view="view"></slot>
											</div>
										</div>
									</div>
								`,
									...
									methods: {
											...
											create() {
													...
													this.status = true;
													this.$nextTick(() => {
															this.view = layer.tips('<div id="content"></div>', '#btn-confirm', {
																	time: 0,
																	area: ['auto', '140px'],
																	shade:[0,'rgba(255,255,255,0)'],
																	shadeClose: true,
																	id: 'confirm',
																	tips: [3, '#fff'],
																	success: (layero, index) => {
																			$(layero).css('box-shadow', '0 0 15px rgb(189 189 189)');
																			document.getElementById('content').appendChild(that.$refs.content);
																			$('.layui-layer-btn1').off().click(() => {
																					that.open()
																					layer.close(that.view);
																			})
																	},
																	yes: () => {
																			that.$emit('yes', that.view);
																	},
																	end: () => {
																			that.status = false;
																	}
        ...
        template: `<div class="model_project_log"><div class="mb10">日志路径<input type="text" name="model_log_path" id="model_log_path" :value="path" v-model="path" placeholder="日志路径" class="ml10 bt-input-text mr10" style="width:350px" />\
		<span class="glyphicon glyphicon-folder-open cursor mr10" @click="change_path"></span>\
		<button class="btn btn-success btn-sm" name="submitLogConfig" @click="change_log_path">保存</button>\</div>\
		<div class="mb10" style="line-height: 32px;">日志大小<span class="ml10">{{size}}</span></div>\
		<div class="inlineBlock log-split" style="line-height: 32px;" @mouseenter="onEnter" @mouseleave="onLeave">\
			<label @click="set_log_split" >\
				<div :class="'bt-checkbox ' + (rdata.status && rdata.data.status ? 'active':'') "></div>\
				<span>日志切割</span>\
			</label>\
			<span class="unit">{{html}}，如需修改请点击<a href="javascript:;" @click="mamger_log_split" class="btlink">编辑配置</a></span>\
		</div>\
		<ul class="help-info-text c7 ">\
			<li>修改日志路径会重启项目</li>\
			<li>Tomcat内置项目，修改日志路径会对当前版本Tomcat的所有站点生效，不能单独设置</li>\
		</ul>\
		<button @click="get_project_logs" class="refresh_log btn btn-sm btn-default" style="margin-bottom:10px">刷新日志</button>\
		<div @mouseenter="onShowButton('show')" @mouseleave="onShowButton('hide')" style="position:relative">\
		<pre class="command_output_pre" style="height:450px;" ref="command" >{{command}}</pre>\
		<button title="全屏展示日志" @click="full_show_detail" class="full full_show_detail btn btn-sm"><span class="full-icon"></span></button></div></div>`,
                    props: {
                        data: Object,
                    },
                    data: function () {
                    ...
                    this.get_log_split();
                        ...
                        async set_log_split() {
                            let that = this;
                            if (this.rdata['is_old']) {
                                bt_tools.msg(this.rdata);
                                return;
                            }
                            bt.confirm(
                                {
                        ...
                        async change_log_path () {
                            if (!this.path) {
                                bt.msg({ status: false, msg: '日志路径不能为空' });
                                return;
                            }
                            let res = await this.$http({
                                change_log_path: '修改项目路径',
                            },{
                                project_name: this.data.name,
                                path: this.path
                            });
                        ...
                        async change_path () {
                            let that = this;
                            bt.select_path('model_log_path','dir',function (val) {
                                that.path = val
                            })
                        },
												async onShowButton(type){
													if(type == 'show'){
														$('.full').show()
													}else{
														$('.full').hide()
												...
												async full_show_detail () {
													...
													bt_tools.open({
														...
														title: '查看日志',
														closeBtn: 1,
														content: '<pre style="background:black;color:white;height:100%;margin:0;font-size:14px">'+this.command+'</pre>', // 内容
                ...
                Vue.component('bt-site-service-status', {
                        ...
                        <div class="sfm-opt">
                            ...
                            <bt-button title="重启" @click="set_service_status('restart_project',data.name)"></bt-button>
														...
														<div class="outer_state_alert">\
																<input class="btswitch btswitch-ios isServerStop" :data-project="data.id" data-name="data.name" :data-status="data.run" data-type="java" type="checkbox" id="isJavaStop"><label class="btswitch-btn" for="isJavaStop"></label>\
														</div>\
														<span class="btlink serverStopSet" :data-project="data.id" :data-name="data.name" :data-status="data.run" data-type="java" style="margin-left:10px;font-size:14px">告警设置</span>\
												</div>\
                    </div>`,
                    props: {
                        data: {
                    ...
                    mounted() {
											site.server_status_event({project: this.data.id,name: this.data.name,status: this.data.run,type: 'java'},true)
										},
                });
                ...
                Vue.component('bt-java-form', {
                    template: `<div :class="{'bt-form':true,'ptb15':!isAddForm}">
                            ...
                            <bt-line label="Tomcat版本" v-if="form.project_type < 3 && $root.updateView" >
                                ...
                                <bt-link title="安装Tomcat其他版本" v-if="isAddForm" @click="$root.tomcat_manage_view(true)"></bt-link>
                            </bt-line>
                            ...
                            <bt-line label="项目端口" v-if="form.project_type > 1">
																<bt-text width="180px" type="number" v-model="form.port"
																		:placeholder="form.project_type < 3?'请输入项目端口':'选择项目文件，自动获取端口'" @input="input_project_port" @blur="blur_project_port"/>
																		<template v-if="isAddForm">
																				<bt-checkbox class="port-open ml10" v-model="form.release_firewall" label="放行端口"/>
																				<a class="bt-ico-ask" @mouseenter="onEnter" @mouseleave="onLeave">?</a>
																		</template>
																</bt-text>
                                <span></span>
                            </bt-line>
                            ...
                            <bt-line label="项目JDK" v-if="form.project_type > 2">
                                <bt-select width="300px" v-model="form.project_jdk" @change="cut_project_jdk" v-if="$root.updateView">
                                    <bt-option v-for="(item,index) in jdkList" :key="index" :title="item.title" :value="item.value"></bt-option>
																...
																<bt-link title="添加JDK信息" v-if="isAddForm" @click="$root.javaEnvShow=true"></bt-link>
                            </bt-line>
                            <bt-line label="项目执行命令" v-if="form.project_type > 2">
                                <bt-text type="textarea" width="380px" line-height="20px" height="70px"
                                         ...
                                         placeholder="选择项目jar文件，自动获取项目执行命令"/>
														...
														<bt-line label="jmx 监控" v-if="form.project_type > 2">
																<bt-checkbox @change="set_jmx_status" :showTips="isAddForm" :disabled="form.project_cmd === ''" :tips="'请选择Java项目后，重新尝试'" v-model="form.jmx_status" label="设置jmx后可在性能监控中查询项目性能信息"/>
														</bt-line>
                                ...
                                <bt-checkbox v-model="form.auth" label="是否设置开机自动启动（默认自带守护进程每120秒检测一次）"/>
                    					...
                    					<bt-text width="380px" v-model="form.project_ps"
                             					placeholder="请输入项目备注,非必填"/>
															...
															<div v-show="form.project_type===3 && isAddForm" class='demo_hr_div '>
																<span class='btlink form-check-lable-receive other_disposition active' @click="advanced_options">{{showMore}}<span :class="!advanced_settings?'select-icon':'select-icon-roate'"></span></span>
															</div>
														<div v-show="advanced_settings || !isAddForm" class="item-content">
                    ...
                    methods: {
                         ...
                         */
                        input_project_name() {
                            if(this.isAddForm) this.form.project_ps = this.form.project_name;
                            if (this.form.is_separation == 1) {
                                this.form.static_path = '/www/wwwroot/' + this.form.project_name;
                            }
                        },
												async onEnter() {
													layer.tips('选中将在防火墙安全组放行监听端口，放行后该项目可在外网访问', this.$el.querySelector('.bt-ico-ask'), {tips: [2, '#20a53a'], time: 0});
												},
												async onLeave() {
																layer.closeAll('tips');
												},
                        ...
                        async set_project_debug() {
                            if (this.form.debug) {
                                ...
                                await this.get_java_project_cmd(this.form, 3);
												 ...
												 * @description 设置jmx
												 */
												async set_jmx_status(){
													if(this.form.jmx_status){
															try{
																	await this.get_java_project_cmd(this.form, 4);
															}catch(e){
																	this.form.jmx_status = false;
															}
													}else{
															await this.get_java_project_cmd(this.form, 4);
													}
                        ...
                        async blur_project_port() {
														...
														setTimeout(async function () {
																if(that.form.port!=''){
																		await that.cheek_java_port(that.form.port);
																}
														},500)
                        ...
                        async cut_project_jdk() {
                            ...
                            await this.get_java_project_cmd(this.form, 2);
												...
												async cheek_java_port(port){
													let time = 0;
													try {
															let rdata = await this.$http(
																{
																		advance_check_port: '检测端口是否被占用',
																		verify: false,
																},
																{
																		port: port,
															...
															if (!rdata.status) {
																	...
																	this.portOpenShow=false
																	bt_tools.msg(rdata.msg, 2);
															}else {
																	this.portOpenShow=true
															}
													} catch (e) {
															this.$msg(e);
													}
                        ...
                        async get_java_project_cmd(param, type = 0) {
                            try {
                                let rdata = await this.$http(
                                    ...
                                    },
                                    {
                                        ...
                                        project_cmd: param.project_cmd,
																				jmx_status:param.jmx_status?true:false,
                                ...
                                this.form.project_cmd = rdata.msg;
																...
																let jmx_port = rdata.msg.substr((rdata.msg.indexOf(port_msg)+port_msg.length)+1,4)
																if(Number(jmx_port) !== 'NaN'){
																		this.form.jmx_port = jmx_port
																}
                        ...
                        async editing_project_info() {
                            try {
															this.form.jmx_status = this.form.jmx_status?true:false
															this.form.jmx_port = (Number(this.form.jmx_port))?this.form.jmx_port:''
                                let rdata = await this.$http(
                                    {
                                ...
                                this.$msg(rdata);
																this.$root.projectInfo = this.$parent.data;
																this.form.jmx_status = this.form.jmx_status?1:0
                                this.$parent.config &&
                    ...
                    mounted() {
                        if (this.$parent.config && this.$parent.data) {
                            ...
                            let project_config = row.project_config;
														// 预处理端口数据
														let port_msg = 'Dcom.sun.management.jmxremote.port'
														let jmx_port = project_config.project_cmd?.substr((project_config.project_cmd.indexOf(port_msg)+port_msg.length)+1,4)
                            ...
                            this.form = Object.assign(config.form, {
                                ...
                                port: project_config.port, //项目端口
																jmx_status:project_config.jmx_status?1:0,// 是否开启jmx设置
                                ...
                                debug: row.debug ? 1 : 0,
								    ...
								    // 网站管理 -JAVA模块- JAVA环境配置-JDK配置
								Vue.component('bt-java-jdk', {
									template:`
										<div>
											<div style="display:flex;flex-direction: row;justify-content: space-between;">
													<bt-layer-btn-confirm title="添加自定义JDK" @yes="site.javaModle.add_jdk_info">
														<template v-slot:default="slotProps">
														<div style="display: flex;flex-flow: column;align-items: flex-start;">
																<span class="mb10">JDK路径：</span>
																<bt-text width="250px" v-model="site.javaModle.jdkPathValue" class-name="mr10" placeholder="请输入自定义JDK地址，或选择JDK路径"
																				icon="folder-open" @icon-event="site.javaModle.select_path('add_load_jdk', 'file')"></bt-text>
																<bt-button class="mt10" type="success" title="添加" @click.native="add_jdk_info(slotProps.view)" style="margin-left: 234px;"></bt-button>
														</div>
														</template>
													</bt-layer-btn-confirm>
													<bt-button class="ml10" type="default" title="刷新列表" @click.native="site.javaModle.update_jdk"></bt-button>
											</div>
											<div class="divtable mt10" style="max-height: 380px;" v-bind:scroll-top="scrollTop" v-on:scroll="update_scroll_top">
													<table class="table table-hover">
															<thead ref="table_thead" style="position: relative;z-index: 1;">
															<tr ref = "table_tr">
																	<th>JDK版本</th>
																	<th>JDK地址</th>
																	<th style="text-align:right;width:132px;">操作</th>
															</tr>
															</thead>
															<tbody>
															<tr v-for="(item,index) in site.javaModle.jdkInfo" :key="index">
																	<td><span>{{item.name}}</span></td>
																	<td v-if="item.operation!==2"><bt-link :title="item.path" :is-info="true"></bt-link></td>
																	<td v-else><span>{{item.path}}</span></td>
																	<td style="text-align:right;">
																			<template v-if="item.operation===1">
																					<bt-link title="删除" @click.native="site.javaModle.delete_jdk_info(item,index)"/>
																			</template>
																			<template v-else-if="item.operation===2" >
																					<span>不可操作</span>
																			</template>
																			<template v-else-if="item.operation===3" >
																					<bt-link title="正在安装，点击刷新" @click.native="site.javaModle.update_jdk"/>
																			</template>
																			<template v-else-if="item.operation===4" >
																					<bt-link title="正在安装...."/>
																			</template>
																			<template v-else >
																					<bt-link title="安装" @click.native="install_jdk(item,index)"/>
																			</template>
																	</td>
															</tr>
															</tbody>
								...
								`,
									...
									data() {
											return {
													jdkPathValue: site.javaModle.jdkPathValue,
													scrollTop: site.javaModle.scrollTop,
													jdkInfo: site.javaModle.jdkInfo,
											}
									},
									methods:{
											select_path(type, file) {
													site.javaModle.select_path(type, file);
											},
											add_jdk_info(view) {
													site.javaModle.add_jdk_info(view);
											},
											update_jdk() {
													site.javaModle.update_jdk();
											},
											/**
											 * @description 获取滚动条位置
											 */
											update_scroll_top(event) {
													this.scrollTop = event.target.scrollTop;
													if(this.scrollTop==0){
															this.$refs['table_thead'].style.transform = `translateY(${this.scrollTop}px)`; // 同步表头位置
													}else {
															this.$refs['table_thead'].style.transform = `translateY(${this.scrollTop-0.5}px)`; // 同步表头位置
													}
											},
											delete_jdk_info(item, index) {
													site.javaModle.delete_jdk_info(item, index);
											},
											/**
											 * @description 安装jdk
											 */
											async install_jdk(item,index) {
													// 判断是否选中
													item.operation=4
													try {
															let data = await site.javaModle.set_default_jdk({
																	name: 'jdk',
																	version: item.name,
																	type: 1,
															})
															this.$forceUpdate()
															await site.javaModle.update_jdk()
													} catch (error) {
															bt_tools.msg(error)
															console.log(error)
													}
							...
							Vue.component('bt-java-tomcat', {
									template: `
								<div>
									<div class="box_item" v-for="(item,key) in site.javaModle.tomcatInfo">
										<div class="box_title" @click="collapse_panel(key)">
											<span>{{key}}</span>
											<span v-show="item.status" :ref="key+'type'" style="margin-right: 10px">收起<span style="margin-left: 5px" class="glyphicon glyphicon-menu-down" aria-hidden="true"></span></span></div>
											<template v-if="item.status">
												<div :ref="key" class="block" >
														<div class="status">
																<div class="btn_item_block">
																		<span>当前状态：<bt-link :title="item.info.status?'运行中':'已停止'"
																														...
																														@click="site.javaModle.set_tomcat_service(item.version,item.info.status?'stop':'start')"/></span>
																</div>
																<div class="btn_item_block">
																		<span>端口：</span>
																		<span>{{item.info.port}}<bt-link title="更改"
																																		@click="site.javaModle.open_tomcat_port_view(item.version,item.info.port)"
																																		class="ml5"/></span>
																</div>
																<div class="btn_item_block">
																		<span>JDK路径：</span>
														...
														<bt-button @click="site.javaModle.set_tomcat_service(item.version)" title="启动"
																				v-if="!item.info.status"></bt-button>
														<bt-button @click="site.javaModle.set_tomcat_service(item.version,'stop')" title="停止"
																				v-if="item.info.status"></bt-button>
														<bt-button @click="site.javaModle.set_tomcat_service(item.version,'restart')"
																				title="重载配置"></bt-button>
														<bt-button type="danger"
																				@click="site.javaModle.install_tomcat_version(item.version,'uninstall')"
																				title="卸载"></bt-button>
												</div>
										</template>
										<template v-else>
												<div ref="key" class="block"><span style="margin-right: 15px">当前版本未安装，点击安装后操作</span>
														<bt-button size="xs" title="点击安装"
																				@click="site.javaModle.install_tomcat_version(item.version)"></bt-button>
												</div>
										</template>
									</div>
								</div>`,
									methods: {
											 ...
											 * @description 折叠面板
											 */
											async collapse_panel(key) {
													var that= this
													$(this.$refs[key]).slideToggle(200,function (){
															if($(that.$refs[key]).css("display")=='none'){
																	$(that.$refs[key]).prev().find('span').next().html('展开<span style="margin-left: 5px" class="glyphicon glyphicon-menu-right" aria-hidden="true"></span>')
															}else{
																	$(that.$refs[key]).prev().find('span').next().html('收起<span style="margin-left: 5px" class="glyphicon glyphicon-menu-down" aria-hidden="true"></span>')
															}
													})
											},
									}
