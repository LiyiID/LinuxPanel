var auto_area = {
	run: function () {
		// 运行应用
		var run = $('.data-search input[name=searcharea]'),
			runList = $('.searchList'),
	...
	appRunList: function (runList, v) {
		...
		for (i in searchValue) {
			...
			if (
				runSearchName.indexOf(v) >= 0 || // 搜索关键词
				runSearchPinyin.indexOf(v) >= 0 ||
				runSearchPy.indexOf(v) >= 0 ||
				runSearchPinyin.toLowerCase().indexOf(v) >= 0 ||
				runSearchPy.toLowerCase().indexOf(v) >= 0
			) {
				loaded[i] = 1;
				temp +=
					'<a class="area_menu" href="javascript:;" data-flag=1 data-code="' +
					runSearchCode +
					'" data-name="' +
					runSearchName +
					"\" onclick=\"selectProvince('sub',this,'')\"><em>" +
					runSearchPinyin.replace(v, '<b>' + v + '</b>') +
					'</em>' +
					runSearchName.replace(v, '<b>' + v + '</b>') +
					'</a>';
				if (++n > 10) break;
			}
		}
