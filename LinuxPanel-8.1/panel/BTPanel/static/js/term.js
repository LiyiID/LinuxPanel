          ...
          if (!$(this).index()) {
              that.get_command_find(title, function (rdata) {
                  ...
                  that.editor_command_view({
                      form: rdata,
                      config: { btn: '保存', title: '编辑常用命令信息【' + $('<div></div>').text(title).html() + '】' }
                  });
              });
          } else {
  ...
  reader_command_list: function (callback) {
      ...
      this.get_command_list(function (rdata) {
          ...
          var clipboard = function (text, event) {
              const cb = new ClipboardJS('.tootls_commonly_list li', {
                  text: function(){
                      return text
                  }
              })
          ...
          bt.each(rdata, function (index, item) {
            var title = $('<div></div>').text(item.title).html()
              html += '<li id="'+index+'" data-title="' + title + '" data-index="' + index + '"><i></i><span>' + title + '</span><span class="tootls">' +
                  ...
                  '</span></li>';
              $('.tootls_commonly_list').on('click','#'+index,function (e) {
                  clipboard(item.shell, e)
              })
          });
