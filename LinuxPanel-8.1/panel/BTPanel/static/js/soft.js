var soft = {
  ...
  ignore_list: [], // 忽略列表
	get_ignore_list: function () {
		var _this = this;
		bt_tools.send({url: 'plugin?action=get_plugin_ignore'},function (res) {
			_this.ignore_list = res;
		})
	},
  get_list: function (page, type, search) {
    this.get_ignore_list()
    ...
    if (page == 0) page = bt.get_cookie('load_page');
    if (type == '11') {
      soft.get_dep_list(1);
    ...
    bt.soft.get_soft_list(page, type, search, function (rdata) {
      ...
      rdata.type.unshift({
        ...
        title: '已安装'
			...
			if(rdata.search_history?.length > 0){
				$('#historySearch').parent().show()
				$('#item_title_hot').parents('.records_search_item').css('margin-top', '20px')
				$('#historySearch').empty()
				$.each(rdata.search_history, function (index, item) {
					if(index >= 10) return
					$('#historySearch').append('<div style="font-size:12px;padding: 0 10px;" class="body_item" data-val="' + item.val + '">' + item.val + '</div>');
				});
				$('#historySearch').off('click').on('click', '.body_item', function () {
					$('#SearchValue').val($(this).data('val'));
					soft.get_list(0, type, $(this).data('val'));
          ...
          setTimeout(function () {
            $('#records_search').css('display', 'none');
				...
				$('#historySearch').parent().hide()
				$('#item_title_hot').parents('.records_search_item').css('margin-top', '0px')
			}
      ...
      _this.refresh_table(page, type, search, rdata);
      if (!data.length) {
        $('.soft_card_page').html('<div class="empty_tip">当前暂无数据</div>').css('justify-content', 'center')
      }
      soft.refresh_data = data;
    ...
    $('.ser-text').focus(function () {
      $('.soft-search').css('border', '1px solid #20a53a')
    })
    $('.ser-text').blur(function () {
      $('.soft-search').css('border', '1px solid rgb(170, 170, 170)')
    })
    // 视图调整
    $('.cut_view_model').on('click', function () {
      var type = $(this).data('type');
      $(this).addClass('active').siblings().removeClass('active')
      bt.set_cookie('softShowType', type);
      if (type == 'card') {
        $('.soft_card_page').removeClass('hide')
        $('.old_table_view').addClass('hide')
      } else {
        $('.soft_card_page').addClass('hide')
        $('.old_table_view').removeClass('hide')
      }
    });
    window.onresize = function () {
      if (window.innerWidth < 1250 && $('.commonly_software_list').find('.item').length > 3) {
        for (var i = 5; i > 2; i--) {
          $('.commonly_software_list').children().eq(i).addClass('hide')
        }
      } else {
        $('.commonly_software_list').find('.item').removeClass('hide')
      }
    }
  },
  // 查找正在安装软件的状态
  ...
  refresh_table: function (page, type, search, rdata) {
    ...
    var _tab = bt.render({
      ...
      columns: [{
        ...
        templet: function (item) {
          ...
          return '<span ' + click_opt + ' ' + sStyle + ' ><img class="sort_img_err" alt="" ' + (item.type === 10 ? 'style="height:20px;width:22px"' : '') + ' src="/static/img/soft_ico/ico-' + fName + '.png">' + (!is_php || !item.version?item.title:'PHP-'+ item.version) + ' ' + version + (item.is_beta ? '-<span style="color:#FC6D26">[测试版]</span>' : '') + '</span>';
        }
      },
        {
          field: 'price',
          ...
          templet: function (item,index) {
            ...
            if (item.endtime < 0 && item.pid > 0) {
              ...
              if (typeof item.preview_url != "undefined" && item.preview_url != "") {
                pay_opt = '<a class="btlink soft_hover" href="' + item.preview_url + '" target="_blank">预览</a> | '
              }
              if (item.type != 10) {
                pay_opt += '<a class="btlink soft_hover" onclick=\'bt.soft.product_pay_view(' + JSON.stringify({
                  name: item.title,
                  pid: item.pid,
                  type: item.type,
                  plugin: true,
                ...
                pay_opt += '<a class="btlink soft_hover" onclick="bt.soft.re_plugin_pay_other(\'' + item.title + '\',\'' + item.pid + '\',' + re_status + ',' + item.price + ')">' + re_msg + '</a>';
              }
            ...
            if (webcache && is_php) {
                    ...
                    if (pay_opt == '') {
                      if (item.versions.length > 1) {
                        for (var i = 0; i < item.versions.length; i++) {
                          ...
                          if (ret > 0) {
                            // 判断是否存在忽略列表
														var ignoreArr = _this.ignore_list.filter(function (ignore_item) {
															return ignore_item.name == item.name && ignore_item.version == min_version.m_version + '.' + min_version.version;
														})
                            if (ret == 2 && ignoreArr.length == 0) option += '<a class="btlink" onclick="bt.soft.update_soft(\'' + item.name + '\',\'' + item.title + '\',\'' + min_version.m_version + '\',\'' + min_version.version + '\',\'' + min_version.update_msg.replace(/\n/g, "_bt_") + '\',\'' + item.type + '\')" >更新</a> | ';
                            break;
                          }
                        }
                      } else {
                        ...
                        // 判断是否存在忽略列表
												var ignoreArr = _this.ignore_list.filter(function (ignore_item) {
													return ignore_item.name == item.name && ignore_item.version == cloud_version;
												})
                        if (item.version != cloud_version && is_beta == item.is_beta && ignoreArr.length == 0) option += '<a class="btlink" onclick="bt.soft.update_soft(\'' + item.name + '\',\'' + item.title + '\',\'' + min_version.m_version + '\',\'' + min_version.version + '\',\'' + min_version.update_msg.replace(/\n/g, "_bt_") + '\',\'' + item.type + '\')" >更新</a> | ';
                      }
                      if (item.admin) {
                        option += '<a class="btlink soft_hover" onclick="bt.soft.set_lib_config(\'' + item.name + '\',\'' + item.title + '\',\'' + item.version + '\')">' + lan.soft.setup + '</a> | ';
                      } else {
                        option += '<a class="btlink soft_hover" onclick="soft.set_soft_config(\'' + item.name + '\')">' + lan.soft.setup + '</a> | ';
                      }
                    } else {
                      option = pay_opt + ' | ' + option;
                    }
                    option += '<a class="btlink soft_hover" onclick="bt.soft.un_install(\'' + item.name + '\')" >' + lan.soft.uninstall + '</a>';
                  } else if (item.task == '-1') {
                    option = '<a class="btlink soft_hover" onclick="messagebox()">正在安装</a>';
                  } else if (item.task == '0') {
                    option = '<a class="btlink soft_hover" onclick="messagebox()">等待安装</a>';
                  } else if (item.task == '-2') {
                    option = '<a class="btlink soft_hover" onclick="messagebox()">正在更新</a>';
                  } else {
                    if (pay_opt) {
                      option = pay_opt;
                    } else {
                      option = '<a class="btlink soft_hover" onclick="bt.soft.install(\'' + item.name + '\',this)"  >' + lan.soft.install + '</a>';
                    }
                  }
                }
              }
              ...
              if (rdata.apache22 && is_php && $.inArray(item.name, phps) == -1) {
                ...
                if (item.setup && item.task == '1') {
                  ...
                  if (pay_opt == '') {
                    ...
                    if (item.versions.length > 1) {
                      ...
                      if (item.admin === true) {
                        ...
                        // 判断是否存在忽略列表
												var ignoreArr = _this.ignore_list.filter(function (ignore_item) {
													return ignore_item.name == item.name && ignore_item.version == up_version.m_version + '.' + up_version.version;
												})
                        if (is_update && ignoreArr.length == 0) option += '<a class="btlink soft_hover" onclick="bt.soft.update_soft(\'' + item.name + '\',\'' + item.title + '\',\'' + up_version.m_version + '\',\'' + up_version.version + '\',\'' + up_version.update_msg.replace(/\n/g, "_bt_") + '\',\'' + item.type + '\')" >更新</a> | ';
                        ...
                        for (var i = 0; i < item.versions.length; i++) {
                          ...
                          if (ret > 0) {
                            ...
                            if (ret == 2 && ignoreArr.length == 0) option += '<a class="btlink soft_hover" onclick="bt.soft.update_soft(\'' + item.name + '\',\'' + item.title + '\',\'' + min_version.m_version + '\',\'' + min_version.version + '\',\'' + min_version.update_msg.replace(/\n/g, "_bt_") + '\',\'' + item.type + '\')" >更新</a> | ';
                            break;
                          }
                        }
                      }
                    ...
                    if (item.type !== 5 && item.type !== 10) option += '<a href="javascript:;" class="btlink soft_hover" onclick="bt.soft.plugin_toolbox_info(\'' + item.name + '\',\'' + item.title + '\',\'' + item.version + '\',' + item.is_beta + ','+ index +')">修复</a> | ';
                  } else {
                    option = pay_opt + ' | ' + option;
                  }
                  option += '<a class="btlink soft_hover" onclick="bt.soft.un_install(\'' + item.name + '\')" >' + lan.soft.uninstall + '</a>';
                  ...
                  option = '<a class="btlink soft_hover" onclick="messagebox()"  >正在安装</a>';
                  soft.is_install = true;
                  bt.set_cookie('is_install', soft.is_install);
                } else if (item.task == '0') {
                  option = '<a class="btlink soft_hover" onclick="messagebox()"  >等待安装</a>';
                  soft.is_install = true;
                  bt.set_cookie('is_install', soft.is_install);
                  ...
                  if (item.route) {
                    pay_opt = '<a class="btlink soft_hover" href="' + item.route + '" target="_self">立即前往</a>';
                  }
                  if (pay_opt) {
                    option = pay_opt;
                  } else {
                    if (typeof item.preview_url != "undefined" && item.preview_url != "") {
                      option = '<a class="btlink soft_hover" href="' + item.preview_url + '" target="_blank">预览</a> | '
                    } else {
                      option = ''
                    }
                    option += '<a class="btlink soft_hover" onclick="bt.soft.install(\'' + item.name + '\',this)"  >' + lan.soft.install + '</a>';
            ...
            // 增加鼠标移入块
            var fName = item.name;
            if (bt.contains(item.name, 'php-')) fName = 'php';
            option =
                '<div style="position:relative" class="soft_hover_' +
                item.id +
                '">\
             ' +
                option +
                '\
                <div class="hide soft_item">\
                    <img class="sort_img_err" style="width:22px;" alt="" src="/static/img/soft_ico/ico-' +
                fName +
                '.png">\
                        <span style="">' +
                item.title +
                '</span>\
                        <span class="soft_item_tri hide"><span>\
                </div>\
             </div>';
            return option;
          }
    ...
    $('.soft_hover').mouseover(function (e) {
      var xPos = parseInt(e.offsetX),
          yPos = parseInt(e.clientY - 124) + 'px';
      $(this).parent().find('.soft_item').removeClass('hide');
      $(this)
          .parent()
          .find('.soft_item')
          .css({
            left: xPos - $(this).parent().find('.soft_item').innerWidth() + 'px',
            top: '-' + parseInt($(this).parent().find('.soft_item').innerHeight() + 10) + 'px',
          });
      $(this).parent().find('.soft_item_tri').removeClass('hide');
      $(this)
          .parent()
          .find('.soft_item_tri')
          .css({
            left: parseInt($(this).parent().find('.soft_item').innerWidth() - 20) + 'px',
            top: parseInt($(this).parent().find('.soft_item').innerHeight()) + 'px',
          });
    });
    // 鼠标移出隐藏
    $('.soft_hover').mouseleave(function (e) {
      $(this).parent().find('.soft_item').addClass('hide');
      $(this).parent().find('.soft_item_tri').addClass('hide');
    });
  },
  ...
  set_soft_tips: function (rdata, type) {
    ...
    var remarks = rdata.remarks,
      tips_info = $('<div class="alert ltd" style="margin-bottom:0"><div class="soft_tips_text"></div><div class="btn-ground" style="display:inline-block;"></div><div class="close_recommend hide"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span><span>关闭推荐</span></div></div>'),
    ...
    if (type === 10) {
      ...
      $.each(remarks.wx_list, function (index, wx_item) {
        wx_li += '<div class="kf_fixed">\
            ...
            <span id="wx_item" class="btlink" style="font-size:13px;">' + wx_item.ps + '</span>\
        ...
        var html = '\
          <div id="customer" style="\
            ...
            <div style="\
              ...
              <a href="https://www.bt.cn/new/wechat_customer" target="_blank" class="btlink" style="font-size: 16px;display: flex;flex-direction: row;flex-wrap: nowrap;align-items: center;"><span style="border-bottom: 1px solid;">点击咨询客服</span><div class="icon-r1" style="width: 15px;height: 18px;margin-top: 1px;margin-left: 5px;"></div></a>\
            </div>\
            ...
            <div class="wechat-title" style="width: 103px;\
    height: 26px;\
    font-size: 11px;\
    font-weight: 700;\
    color: #666;\
    ...
    align-items: center;">\
              ...
              <span class="scan-title" style="font-size: 16px;color: #7e7e7e">扫一扫</span>\
            </div>\
         </div>\
         '
        id = layer.tips(html, '#wx_item', {
          tips: [3, 'rgb(255, 255, 255)'],
          time: 0,
          id: 'kf_fixed',
          shade: [0, '#fff0'],
          shadeClose: true,
          success: function (layero, index) {
            ...
            $('.layui-layer-TipsG').addClass('layui-layer-TipsG1')
            $('#kf_fixed').css({'background': 'transparent', 'box-shadow': 'none'})
          }
        })
        e.stopPropagation();
      })
  ...
  flush_cache: function () {
    bt.soft_flush_cache = 1;
    soft.get_list();
    bt.soft.set_pay_status_icon()
  },
    ...
    switch (key) {
        ...
        bt.pub.get_ftp_logs(function (_status) {
          ...
          if (ltd < 0) {
            tabCon.append('<div class="mask_layer">\
              <div class="prompt_description" style="margin-top: -60px;"><i class="layui-layer-ico layui-layer-ico0" style="width: 20px;height: 20px;display: inline-block;margin-right: 15px;vertical-align: middle;background-size: 700%;"></i>此功能为企业版专享， <a class="btlink" onclick="bt.soft.product_pay_view({totalNum:56,limit:\'' + "ltd" + '\',closePro:false})">立即购买</a></div></div>')
          }
        })
        break;
      case 'service':  // nginx服务
			// 按钮组 重启、停止、重载
        var tabCon = $(".soft-man-con").empty();
        var status_list = [{
        ...
        if (data.name == 'phpmyadmin') {
          ...
          tabCon.append(btns);
					if(data.name == 'nginx' || data.name == 'apache' || data.name == 'mysql' || data.name == 'pureftpd'){
						var alert = $('<div id="alert_setting"></div>')
						tabCon.append(alert);
						bt_tools.send({url:'/push?action=get_push_list'},function(ress){
							...
							$('#alert_setting').html('<span style="margin-right:10px;font-size:14px">状态停止时提醒我:</span><div class="outer_state_alert"><input class="btswitch btswitch-ios" type="checkbox" ' + ((alert_message.status?'checked':'')) + '><label class="btswitch-btn"></label></div> <span class="btlink setting_alert" style="margin-left:10px;font-size:14px">告警设置</span>')
							// 打开详细设置弹窗
							$('.setting_alert').click(function(){
								bt_tools.open({
									area:['500px'],
									skin:'alert_layer',
									btn:['确定','取消'],
									...
									content:{
										...
										form:[
											{
												...
												group: {
													...
													boxcontent:'<div class="mr10 alert_status_input"  style="vertical-align: middle;display: inline-block;">\
														<input class="btswitch btswitch-ios" type="checkbox" '+(alert_message.status?'checked':'')+'>\
												...
												class:'alert_type',
												label:'告警方式',
												group: that.switchPushType(alert_message)
											},{
												group:{
													type:'help',
													style:{
													...
													list:[
														'开启该告警后，若该插件状态停止，将会使用您选中的告警方式发送告警信息',
															'点击安装后状态未更新，尝试点击【<a class="btlink handRefresh">手动刷新</a>】',
													]
											}}
										]
									},success:function(){
										...
										// 间隔时间不可为负数，为负数时，默认调整为60
										$('input[name=interval]').on('input', function () {
											var val = $(this).val();
											if(val < 60 || val === '' || !bt.isInteger(parseFloat(val))){
												$(this).val(600)
												return layer.msg('间隔时间范围应为大于60的正整数',{icon: 2,time: 2000})
											}
										})
										// 点击安装消息通道
										...
										function renderConfigHTML(){
											bt.site.get_msg_configs(function (rdata) {
												...
												for (var key in rdata) {
													var item = rdata[key],_html = '',wx_html='',accountConfigStatus = false;
													if(key == 'sms') continue;
													if(key === 'wx_account'){
														...
														// 微信公众号模块置顶
														wx_html = '<div class="inlineBlock module-check ' + ((!item.setup || $.isEmptyObject(item.data)) ? 'check_disabled' : ((key == 'wx_account' && !accountConfigStatus)?'check_disabled':'')) + '">' +
															'<div class="cursor-pointer form-checkbox-label mr10" style="height:35px">' +
															'<i class="form-checkbox cust—checkbox cursor-pointer mr5 '+ (alert_message.module.indexOf(item.name) > -1?((!item.setup || $.isEmptyObject(item.data)) ?'':((key == 'wx_account' && !accountConfigStatus)?'':'active')):'') +'" data-type="'+ item.name +'"></i>' +
															...
															'<span class="vertical_middle" title="' + item.ps + '"><b class="'+(item.name === 'wx_account'?'':'hide')+'" style="color: #fc6d26;">[推荐]</b>' + item.title + ((!item.setup || $.isEmptyObject(item.data)) ? '[<a target="_blank" class="bterror installNotice" data-type="'+ item.name +'">点击安装</a>]' : ((key == 'wx_account' && !accountConfigStatus)?'[<a target="_blank" class="bterror alertInstall" data-type="'+ item.name +'">未配置</a>]':'')) + '</span>' +
															'</div>' +
															'</div>';
														continue
													}
													_html = '<div class="inlineBlock module-check ' + ((!item.setup || $.isEmptyObject(item.data)) ? 'check_disabled' : ((key == 'wx_account' && !accountConfigStatus)?'check_disabled':'')) + '">' +
															...
															'<i class="form-checkbox cust—checkbox cursor-pointer mr5 '+ (alert_message.module.indexOf(item.name) > -1?((!item.setup || $.isEmptyObject(item.data)) ?'':((key == 'wx_account' && !accountConfigStatus)?'':'active')):'') +'" data-type="'+ item.name +'"></i>' +
															'<input type="checkbox" class="form—checkbox-input hide mr10" name="' + item.name + '" '+ ((item.setup || !$.isEmptyObject(item.data))?((key == 'wx_account' && !accountConfigStatus)?'':'checked'):'') +'/>' +
															'<span class="vertical_middle" title="' + item.ps + '"><b class="'+(item.name === 'wx_account'?'':'hide')+'" style="color: #fc6d26;">[推荐]</b>' + item.title + ((!item.setup || $.isEmptyObject(item.data)) ? '[<a target="_blank" class="bterror alertInstall" data-type="'+ item.name +'">点击安装</a>]' : ((key == 'wx_account' && !accountConfigStatus)?'[<a target="_blank" class="bterror installNotice" data-type="'+ item.name +'">未配置</a>]':'')) + '</span>' +
															'</div>' +
															'</div>';
													if(!item.setup){
														unInstall += _html;
									...
									yes:function(form,index){
										...
										// 判定间隔时间不可为负数
										if(Number(form.interval) < 60 || Number(form.interval) === '' || !bt.isInteger(parseFloat(form.interval))) return layer.msg('间隔时间范围应为大于60的正整数',{icon: 2,time: 2000})
										var params ={
											type:'services',
											project:data.name,
										...
										// 获取选中的告警方式，排除未安装的
										$('.alert_type .info-r').children().each(function(index,item){
        ...
        if (data.name.indexOf('php-') >= 0) {
          ...
          bt.soft.get_soft_find('security_notice', function (rdata_res) {
            if (rdata_res.endtime > 0) {
              //获取全局防护状态
              bt_tools.send({
                url: '/plugin?action=a&name=security_notice&s=get_status',
                data: {}
              }, function (global_config) {
                //获取php版本防护开关
                bt_tools.send({url: '/plugin?action=a&name=security_notice&s=get_index', data: {}}, function (res) {
                  var protective_switch = res.php_versions ? res.php_versions.filter(function (item) {
                    return item.version === data.version
                  })[0] : {}
                  tabCon.append('<div class="php_pro">\
                          <div class="flex align-center justify-between">\
                          <div class="inlineBlock" style="line-height: 32px;">\
                            <span style="vertical-align: middle;">PHP版本防护开关</span>\
                            <div class="phppro-switch ml5" style="float: inherit;display: inline-block;vertical-align: middle;">\
                              <input class="btswitch btswitch-ios" id="isPhpproSwitch" type="checkbox" ' + (protective_switch.state ? 'checked' : '') + '>\
                              <label class="btswitch-btn isPhpproSwitch" for="isPhpproSwitch" style="margin: 0;"></label>\
                            </div>\
                          </div>' +
                    ((!global_config.send || !global_config.status) && protective_switch.state ?
                      '<div class="sn-home--open-condition"><i class="sn-home--important-note">!</i>未启用PHP安全告警，请先' +
                      (global_config.send ? '' : '<span class="set_send">设置告警</span>') + (!global_config.status && !global_config.send ? '、' : '') + (global_config.status ? '' : '开启<span class="set_status">全局监控开关</span>') + '</div>' : '') + '</div><div class="php_site_table"></div>\
                        ' + bt.render_help(['基于PHP内核的监控工具，实时监控网站木马、漏洞等其他入侵行为，发现木马支持自动隔离', '安全告警默认会监视当前网站访问非当前网站文件的行为(例：你的站点A.com访问了站点B.com的文件或者目录)，通过添加监视器路径，当路径被访问时发送告警']) + '</div>')
                  if (!rdata_res.setup) {
                    tabCon.append('<div class="mask_layer">\
                      <div class="prompt_description"><i class="prompt-note">!</i>当前未安装PHP网站安全告警，<a href="javascript:;" class="btlink" onclick="bt.soft.install(`security_notice`,this)">点击安装</a></div>\
                      </div>')
                  }
                  //PHP网站安全告警全局监控开关
                  $('.set_status').on('click', function () {
                    if (!global_config.send) {
                      $('.set_send').click()
                      return layer.msg('未设置告警，请设置后在开启全局监控开关！')
                    }
                    bt_tools.send({
                      url: '/plugin?action=a&name=security_notice&s=start_service',
                      data: {status: 'start'}
                    }, function (set_status_res) {
                      bt_tools.msg(set_status_res)
                  ...
                  $('.set_send').on('click', function () {
                    bt.open({
                      ...
                      shadeClose: false,
                      btn: ['确定', '取消'],
                      content: '<div class="php_set_send"><div class="sn-alarm--setting flex align-center">\
                                    <span style="padding-right: 20px;line-height: 34px;">通知方式</span>\
                                    <div class="sn-alarm--selectBox"></div>\
                                    </div>\
                                  </div>',
                      success: function () {
                        var sendChannel = [{name: '邮件', value: 'mail'}, {
                          name: '微信公众号',
                          value: 'wx_account'
                        }, {name: '企业微信', value: 'weixin'}, {name: '钉钉', value: 'dingding'}, {
                          name: '飞书',
                          value: 'feishu'
                        }]
                        // 获取告警列表
                        bt_tools.send({
                          ...
                          $.post('/plugin?action=a&name=security_notice&s=get_send_config', {}, function (rdata) {
                            loadT.close()
                            var html = '', alarmType = -1, accountConfigStatus = false, flag = true
                            for (var i = 0; i < sendChannel.length; i++) {
                              if (rdata.msg == sendChannel[i].value) alarmType = i
                              if (sendChannel[i].value === 'wx_account') {
                                if (!$.isEmptyObject(alarms[sendChannel[i].value]['data']) && alarms[sendChannel[i].value]['data'].res.is_subscribe && alarms[sendChannel[i].value]['data'].res.is_bound) {
                              ...
                              flag = sendChannel[i].value == rdata.msg && is_config ? false : true
														...
														var send_html = '<div class="sn-alram--span"><span class="sn-alarm--channel">'+(alarmType != -1 ? flag ? sendChannel[alarmType].name : '请选择通知方式' :'请选择通知方式')+'</span>'+
														'<i class="sn-alram--selectIcon glyphicon glyphicon-menu-down"></i></div>'+
														'<ul class="sn-alarm--list">'+html+'</ul>';
                            ...
                            $('.sn-alarm--list').find('.installNotice').unbind('click').click(function (ev) {
                              ...
                              openAlertModuleInstallView(type, '.bt-soft-menu p.bgw');
                            });
                          });
                        })
                      },
                      yes: function (index, layero) {
                        ...
                        bt_tools.send({
                          url: '/plugin?action=a&name=security_notice&s=set_send',
                          data: {type: name}
                        }, function (res) {
                          if (res.status) $('.bt-soft-menu .bgw').click()
                          bt_tools.msg(res)
                          layer.close(index)
                        }, '设置告警')
                      }
                  ...
                  //防护开关
                  $('.isPhpproSwitch').click(function () {
                    var _status = $(this).prev().prop('checked');
                    bt.simple_confirm({
                      title: (_status ? '关闭' : '开启') + '【' + data.name + '】防护',
                      msg: _status ? '关闭后，该版本的站点将不再受到安全防护，是否继续？' : '开启后，该版本的站点将受到安全防护，是否继续？'
                    }, function () {
                      bt_tools.send({
                        url: '/plugin?action=a&name=security_notice&s=set_php_status',
                        data: {php_version: protective_switch.v, enable: _status ? 0 : 1}
                      }, function (rdata) {
                        if (rdata.status) {
                          setTimeout(function () {
                            $('.bt-w-menu.bt-soft-menu p').eq(1).click()
                          }, 500)
                        }
                        bt_tools.msg(rdata);
                      }, (_status ? '关闭' : '开启') + '防护')
                    }, function () {
                      $('#isPhpproSwitch').prop('checked', _status ? true : false);
                    })
                  })
                  //获取对应这个版本下的站点列表
                  if (!rdata_res.setup) {
                    bt_tools.table({
                      ...
                      default: "暂无数据",
                      column: [
                        {fid: 'site_name', title: '站点名称'},
                        {fid: 'open', title: '防护状态', type: 'switch'},
                        {title: '今日触发', type: 'text'},
                        {title: '触发总数', type: 'text'},
                        {
                          title: '操作',
                          type: 'group',
                          width: '100px',
                    ...
                    var siteTbale = bt_tools.table({
                      ...
                      dataFilter: function (data) {
                        var site_list = data.sites.filter(function (item) {
                          return item.version === protective_switch.v
                        })
                        ...
                        {fid: 'site_name', title: '站点名称'},
                        {
                          fid: 'open', title: '防护状态', type: 'switch', event: function (row, index, ev, cname) {
                            if (!protective_switch.state) {
                              ...
                              return layer.msg('请先开启PHP版本防护开关', {icon: 2})
                            }
                            ...
                            bt.confirm({
                              ...
                              $.post({
                                url: '/plugin?action=a&name=security_notice&s=' + (_status ? 'stop_site' : 'start_site'),
                          ...
                          group: [{
                                      ...
                                      column: [
                                        ...
                                        },
                                        {
                                          ...
                                          group: [{
                                            ...
                                            event: function (items) {
                                              bt_tools.send({
                                                url: '/plugin?action=a&name=security_notice&s=del_site_config',
                                                data: {domain: row.site_name, type: items.type, path: items.path}
                                              }, function (rdata) {
                                                bt_tools.msg(rdata)
                                                if (rdata.status) {
                                                  siteTbale.$refresh_table_list();
                                    ...
                                    function edit_file_monitor(rowdata) {
                                      ...
                                      bt_tools.open({
                                          ...
                                          form: [
                                            {
                                              ...
                                              group: {
                                                ...
                                                value: rowdata ? rowdata.path : row.path,
                                                icon: {
                                        ...
                                        success: function (layero, index) {
                                          ...
                                          $('.module-check.check_disabled label').unbind('click').click(function () {
                                            if ($(this).parent().hasClass('check_disabled')) layer.tips('目录暂不支持该告警操作！', $(this), {
                                              tips: [3, '#ff0000'],
                                              time: 3000
                                            })
                                          })
                                            ...
                                            if (rdata.status) {
                                              bt_tools.send({
                                                ...
                                                var site_filter1 = site_res.sites.filter(function (a) {
                                                  return a.path === row.path && a.site_name === row.site_name
                                                })
                            ...
                            event: function (row, index, ev, key, that) {
                              bt_tools.send({
                                ...
                                data: {siteName: row.site_name, page: 1, limit: 10}
                              }, function (res) {
                                    ...
                                    bt_tools.table({
                                      ...
                                      column: [{
                                        ...
                                        event: function (row, index) {
                                          layer.confirm('是否将 <span style="color:#20a53a">' + row['address'] + '</span> 添加到IP白名单？', {
                                            title: '加入IP白名单',
                                            closeBtn: 2,
                                            icon: 0
                                          }, function () {
                                            bt_tools.send({
                                              url: '/plugin?action=a&name=security_notice&s=add_ip_white',
                                              data: {start_ip: row['address'], end_ip: row['address']}
                                            }, function (res) {
                                              bt_tools.msg(res)
                                            })
                                          });
                                        ...
                                        group: [{
                                          title: 'URL加白',
                                          event: function (row) {
                                            layer.confirm('加入URL白名单后此URL将不再进行防御，是否继续操作？', {
                                              title: '加入URL白名单',
                                              icon: 3,
                                              closeBtn: 2
                                            }, function () {
                                              bt_tools.send({
                                                url: '/plugin?action=a&name=security_notice&s=add_url_white',
                                                data: {url_rule: row.url}
                                              }, function (res) {
                                                layer.msg(res.msg, {icon: res.status ? 1 : 2});
                                              })
                                            });
                                              ...
                                              success: function ($layer) {
                                                ...
                                                $layer.find('.btn-error').click(function () {
                                                  layer.confirm('加入URL白名单后此URL将不再进行防御，是否继续操作？', {
                                                    ...
                                                    bt_tools.send({
                                                      url: '/plugin?action=a&name=security_notice&s=wubao_url_white',
                                                      data: {url_rule: row.url}
                                                    }, function (res) {
                                                      layer.msg(res.msg, {icon: res.status ? 1 : 2});
                                                    })
              ...
              tabCon.append('<div class="webedit-con-plugin"><div class="thumbnail-introduce-new">\
                      <div class="thumbnail-introduce-title-new">\
                        <div class="thumbnail-title-left-new">\
                            ...
                            <div class="thumbnail-title-text-new">\
                              <p>PHP网站安全告警-功能介绍</p>\
                              <p>基于PHP内核监控工具，实时监控网站木马、漏洞等其他入侵行为，发现木马支持自动隔离</p>\
                            </div>\
                        </div>\
                        <a class="btn btn-default mr10" title="产品预览" href="https://www.bt.cn/bbs/thread-109515-1-1.html" target="_blank">产品预览</a>\
                        <div class="thumbnail-title-button-new daily-product-buy"><button type="button" class="btn btn-success va0 buyIntrusion" onclick=\'bt.soft.product_pay_view(' + JSON.stringify({
                name: rdata_res.title,
                pid: rdata_res.pid,
                type: rdata_res.type,
                plugin: true,
                renew: rdata_res.endtime,
                ps: rdata_res.ps.replaceAll("'", '`'),
                ex1: rdata_res.ex1,
                totalNum: 113
              }) + ');\'>立即购买</button>\
                        </div>\
                    ...
                    <div class="thumbnail-introduce-ul-new">\
                      <ul>\
                        <li><span class="li-icon"></span>实时监控网站木马</li>\
                        <li><span class="li-icon"></span>实时监控网站漏洞</li>\
                        <li><span class="li-icon"></span>支持自动隔离木马</li>\
                      </ul>\
                    </div>\
                </div>\
                <div class="img_view" style="position: relative;width: 80%;margin: auto;"><img class="product_view_img" src="https://www.bt.cn/Public/new/plugin/introduce/soft/securityNotice.png" style="width: 100%;box-shadow: 0px 6px 10px rgba(145, 145, 145, 0.25);"><div class="img_view_mask"></div></div>\
                </div>')
              // 产品预览
              $('.img_view img,.img_view_mask').click(function () {
                layer.open({
                  ...
                  closeBtn:2,
                  btn:false,
                  content:'<img src="https://www.bt.cn/Public/new/plugin/introduce/soft/securityNotice.png" style="width:100%" />'
                })
              })
              ...
              $('.thumbnail-box .thumbnail-item').unbind('click').on('click', function () {
                var index = $('.thumbnail-tab .on').index();
                bt.open({
                  title: false,
                  btn: false,
                  closeBtn: 1,
                  area: ['900px', '680px'],
                  content: '<img src="/static/images/recommend/securityNotice' + (index ? index + 1 : '') + '.png" style="width:100%;height: 680px;"/>',
      ...
      case 'get_mysql_run_status':
        bt.send('GetRunStatus', 'database/GetRunStatus', {}, function (rdata) {
          ...
          var Con = '<div class="divtable"><table class="table table-hover table-bordered" style="width: 650px;margin-bottom:10px;background-color:#fafafa">\
								<tbody>\
      ...
      case 'get_mysql_status':
        bt.send('GetDbStatus', 'database/GetDbStatus', {}, function (rdata) {
          ...
          var form_datas = [{
          ...
          },
            {
              ...
              width: '70px',
              value: rdata.mem.key_buffer_size,
            ...
            },
            {
              ...
              width: '70px',
              value: rdata.mem.bt_query_cache_size,
            ...
            },
            {
              ...
              name: 'tmp_table_size',
              width: '70px',
              value: rdata.mem.tmp_table_size,
            ...
            },
            {
              ...
              value: rdata.mem.innodb_buffer_pool_size,
              width: '70px',
            ...
            },
            {
              ...
              value: rdata.mem.innodb_log_buffer_size,
              width: '70px',
            ...
            },
            {
              ...
              width: '70px',
              value: rdata.mem.sort_buffer_size,
            ...
            },
            {
              ...
              width: '70px',
              value: rdata.mem.read_buffer_size,
            ...
            },
            {
              ...
              width: '70px',
              value: rdata.mem.read_rnd_buffer_size,
            ...
            },
            {
              ...
              width: '70px',
              value: rdata.mem.join_buffer_size,
            ...
            },
            {
              ...
              value: rdata.mem.binlog_cache_size,
              width: '70px',
        ...
        break;
          ...
          var limitCon = '<p class="conf_p">\
                        <button class="btn btn-default btn-clear btn-xss" style="float:left;" >' + lan.soft.mysql_log_close + '日志</button></p>\
                      <textarea readonly style="margin: 0px;width: 650px;height: 512px;background-color: #333;color:#fff; padding:0 5px" id="error_log"></textarea>\
                  </p>'
          $(".soft-man-con").html(limitCon);
          //清空日志
          $(".btn-clear").click(function () {
            bt.simple_confirm({title: '清空日志操作提示', msg: '清空日志会导致，运行错误信息丢失，无法排除问题，请谨慎操作，是否继续？'},function (index) {
              ...
              $.post('/database?action=GetErrorLog', "close=1", function (rdata) {
                ...
                soft.get_tab_contents('mysql_error_log')
              });
          },function (index) {
              return false
          })
      ...
      case 'mysql_slow_log':
        ...
        bt.send('GetSlowLogs', 'database/GetSlowLogs', {}, function (logs) {
          ...
          var phpCon = '<textarea readonly="" style="margin: 0px;width: 650px;height: 552px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' + logs.msg + '</textarea>';
      ...
      case 'mysql_log_bin':
        var loadT = bt.load()
        // 渲染二进制内容
        var inHtml = '<p class="conf_p">\
                        <span class="f14 c6" style="width:70px">' + lan.soft.mysql_log_bin + ' </span>\
                        <span style="vertical-align: middle;"><input class="btswitch btswitch-ios" id="btn-bin" type="checkbox"><label class="btswitch-btn" for="btn-bin"></label></span>\
                        <span class="f14 c6 mr20 sizeGb" style="width:70px"></span>\
                        <p class="f14 c6 mtb10" style="border-top:#ddd 1px solid; padding:10px 0">\
                        <p style="margin-bottom:10px">· 温馨提示：Mysql二进制日志是记录当前的操作日志，可用于数据恢复，数据备份。</p>\
                        <div id="bin_table"></div>\
                        <p>'
          $(".soft-man-con").html(inHtml);
          // 生成二进制列表
            bt_tools.table({
              el:'#bin_table',
              url:'/database?action=GetMySQLBinlogs',
              default:'暂无数据',
              height: '390px',
              dataVerify: true,  //为false可跳过异常处理
              column:[
                  {
                    fid: 'name',
                    title: '文件名'
                ...
                },
                  {
                    title: '最后修改时间',
                    template:function(row){
                      return '<span>'+ bt.format_data(row.last_modified) +'</span>'
                    }
                  },
                  {
                    title: "操作",
                    ...
                    group: [{
                        title: '删除',
                        hide:function(rows){return rows.current},
                        event: function (row, index, ev, key, that) {
                          bt.show_confirm('删除二进制日志', '您真的要删除[' + row.name + ']吗?', function () {
                            var loadT = layer.msg(lan.public.the_del, {
                              icon: 16,
                              time: 0,
                              shade: 0.3
                            });
                            bt_tools.send({url:'/files?action=DeleteFile',data:{path:row.path}},function(ress){
                              layer.msg(ress.msg, {
        ...
        bt.send('BinLog', 'database/BinLog', {
          ...
          loadT.close()
          $("#btn-bin").prop("checked", rdata.data.binlog_status);
          $(".sizeGb").html(ToSize(rdata.data.size))
        })
        // 点击二进制日志开关
          $("#btn-bin").click(function () {
            if(!$("#btn-bin").prop("checked")){
              var loadC = layer.msg('正在设置中,请稍后...', {
                icon: 16,
              });
              bt.simple_confirm({title: '二进制日志开关操作', msg: '关闭二进制日志后，将不会再记录所有对数据库的修改操作'},function (index) {
                bt_tools.send({url:'/database?action=BinLog'},function(datas){
                  layer.msg(datas.msg, { icon: datas.status ? 1 : 2 });
                  $("#btn-bin").prop("checked", datas.data.binlog_status)
                },{verify:false});
            },function (index) {
              $("#btn-bin").prop("checked", true)
            })
            }else{
              bt.simple_confirm({title: '二进制日志开关操作', msg: '开启二进制日志可以保护Mysql的数据安全，但同时会消耗一定磁盘存储，是否继续?'},function (index) {
                var loadC = layer.msg('正在设置中,请稍后...', {
                  icon: 16,
                });
                bt_tools.send({url:'/database?action=BinLog'},function(datas){
      ...
      function get_phpmyadmin_ssl() {
        ...
        bt.send('get_phpmyadmin_ssl', 'ajax/get_phpmyadmin_ssl', {}, function (tdata) {
          ...
          $('#phpmyadminsafe').prop("checked", tdata.auth);
          if(tdata.auth)$(".user_pw").show();
        ...
        get_phpmyadmin_ssl()
				...
				if(!$("#phpmyadminsafe").prop("checked")){
					$('.user_pw').hide()
				}
        ...
        if (msg == 'close') {
          bt.confirm({
            ...
            bt.send('setPHPMyAdmin', 'ajax/setPHPMyAdmin', {
              ...
              $("#phpmyadminsafe").prop("checked",true)
          })
          return;
        } else {
      ...
      case 'set_fpm_config':
        bt.soft.php.get_fpm_config(version, function (rdata) {
          if(!rdata.status && rdata.msg) bt_tools.msg(rdata)
          var datas = {
            '1GB内存': {
              max_children: 30,
              start_servers: 5,
      ...
      case 'get_fpm_logs':
        bt.soft.php.get_fpm_logs(version, function (logs) {
          var phpCon = '<textarea readonly="" style="margin: 0px;width: '+ (data.name.indexOf('php-') >= 0 ? '640':'500') +'px;height: 560px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' + logs.msg + '</textarea>';
          $(".soft-man-con").html(phpCon);
          var ob = document.getElementById('error_log');
          ob.scrollTop = ob.scrollHeight;
        })
...
bt_tools.send(
	{ url: '/plugin?action=push_plugin' },
	function (ress) {
    if(ress.length>0) {

      $('#item_title_hot').html(ress[0].title);
      $('#item_title_tendency').html(ress[1].title);
      $.each(ress[0].plugin, function (index, item) {
        $('#hotsearch').append('<div style="font-size:12px;padding: 0 10px;" class="body_item" data-ename="' + item.ename + '">' + item.name + '</div>');
      });
      $('#hotsearch').append('<div class="body_item"></div>');
      $.each(ress[1].plugin, function (index, item) {
        $('#tendency').append(
          '<div class="item_box" style="display:flex;align-items:center;font-size:12px;padding: 0 10px;"><img style="width:22px;height:18px" src="/static/img/soft_ico/ico-' +
          item.ename +
          '.png" /><div class="body_item" style="margin-left:5px" data-ename="' +
          item.ename +
          '">' +
          item.name +
          '</div></div>'
      ...
      $('.body_item').click(function (event) {
        bt.soft.get_soft_find($(this).data('ename'), function (dataRes) {
          ...
          } else if (dataRes.setup) {
            if (dataRes.type !== 5) {
              bt.soft.set_lib_config(dataRes.name, dataRes.title, dataRes.version);
            } else {
              soft.set_soft_config(dataRes.name);
            }
            layer.closeAll();
          } else {
            bt.soft.install(dataRes.name)
          }
        });
        // soft.get_list(1, 0, $(this).text());
      });
      $('#SearchValue').click(function () {
        $('#records_search').css('display', 'block');
        $('#records_search').css('opacity', '1');
      });
      $(document).click(function (event) {
        if (!$(event.target).closest('.soft-search').length) {
          $('#records_search').css('opacity', '0');
          setTimeout(function () {
            $('#records_search').css('display', 'none');
          }, 80);
      ...
      $('#all_delete').hover(
        function () {
          $('#all_delete').attr('src', '/static/img/soft_ico/deletes_hover.png');
        },
        function () {
          $('#all_delete').attr('src', '/static/img/soft_ico/deletes.png');
        }
      );
      $('#SearchValue').click(function (event) {
