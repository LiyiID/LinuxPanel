...
admin_path_checks = [
    ...
    '/down', '/api', '/tips', '/message', '/warning', '/bind', '/daily', '/docker',
]
...
def user_Authority():
    ...
    menu = ['/', '/home', '/site', '/ftp', '/database', '/docker', '/control', '/firewall', '/waf', '/files', '/logs', '/xterm', '/crontab', '/soft', '/config', '/site_ifame', '/ftp_ifame',
            '/database_ifame',
            '/docker_ifame', '/control_ifame', '/firewall_ifame', '/waf_ifame', '/files_ifame', '/logs_ifame', '/xterm_ifame', '/crontab_ifame', '/soft_ifame', '/config_ifame']
    uid = session.get('uid')
...
def request_check():
    ...
    if request.path in [
        ...
        '/files', '/xterm', '/crontab', '/config', '/docker', '/logs'
    ]:
...
@app.route('/site_ifame', methods=method_all)
...
@app.route('/database_ifame', methods=method_all)
def database(pdata=None):
    ...
    if request.method == method_get[0] and not pdata:
        ...
        data['js_random'] = get_js_random()
        is_bind()
        if request.path in ['/database_ifame']:
    ...
    defs = ('get_mysql_status', 'GetdataInfo', 'check_del_data', 'get_database_size', 'GetInfo',
            ...
            'set_auto_sqlist', 'set_auto_sqlist', 'GetPushUser', 'ModifyTableComment', 'export_table_structure', 'set_restart_task', 'get_restart_task', 'view_database_types', 'add_database_types',
            'delete_database_types', 'update_database_types', 'set_database_type_by_name', 'find_databases_by_name_and_type'
            )
...
@app.route('/firewall_ifame', methods=method_all)
def firewall(pdata=None):
    ...
    if request.method == method_get[0] and not pdata:
        ...
        if request.path in ['/firewall_ifame']:
...
def files(pdata=None):
    ...
    defs = ('CheckExistsFiles', 'GetExecLog', 'GetSearch', 'ExecShell',
            ...
            'file_history', 'file_history_list', 'del_file_history', 'list_backups', 'restore_backup', 'delete_backup', 'download_backup', 'get_backup_config', 'edit_backup_config',
...
@app.route('/crontab_ifame', methods=method_all)
...
@app.route('/soft_ifame', methods=method_all)
def soft(pdata=None):
    # 软件商店页面
    comReturn = comm.local()
    if comReturn: return comReturn
...
def config(pdata=None):
    ...
    if request.method == method_get[0] and not pdata:
        ...
        if request.path in ['/config_ifame']:
    ...
    defs = (
        ...
        "get_nps_info", "balance_pay_cert", "err_collection"
    )
...
def ajax(pdata=None):
    ...
    defs = ('get_lines', 'php_info', 'change_phpmyadmin_ssl_port',
            ...
            'Set_bt_host', 'Get_ip_info', 'reGetCloudPHPExt', 'GetMysqlAlarmInfo', 'set_cron_task', 'get_cron_task')
...
def system(pdata=None):
    ...
    defs = ('get_io_info', 'UpdatePro', 'GetAllInfo', 'GetNetWorkApi',
            ...
            'ReWeb', 'RestartServer', 'ReMemory', 'RepPanel', 'set_rname',
...
def ssl(pdata=None):
    ...
    defs = ('check_url_txt', 'RemoveCert', 'renew_lets_ssl', 'SetCertToSite',
            ...
            'upload_cert_to_cloud', 'remove_cloud_cert', 'get_ssl_ps', 'set_ssl_ps'
            )
    get = get_input()
...
def panel_cloud(is_csrf=True):
    ...
    if download_url is None:
        return public.returnJson(False, 'download_url为None')
    if plugin_name == 'ftp':
